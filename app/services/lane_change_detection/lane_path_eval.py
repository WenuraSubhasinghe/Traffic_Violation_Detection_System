    ...
    def update(self, detections):
        ...
        for i, obj_box in enumerate(object_boxes):
            for j, det_box in enumerate(input_boxes):
                iou_matrix[i, j] = LanePathEval.compute_iou(obj_box, det_box[:4])
...
class LaneChangeDetector:
    def __init__(self, model_path, confidence=0.5, lane_polygons=None):
        ...
        self.vehicle_sign_history = defaultdict(lambda: [[] for _ in range(len(lane_polygons))])
    ...
    def assign_lanes_and_detect_changes(self, tracking_results, frame_idx, lane_change_events, fps):
        ...
        for object_id, (x1, y1, x2, y2, conf, cls) in tracking_results:
            ...
            for i, poly in enumerate(self.lane_polygons):
                sign, _ = LanePathEval.signed_distance_to_polyline((cx, cy), poly)
...
class LanePathEval:
    ...
    @staticmethod
    def extract_lane_segments_all_frames(model, video_path):
        cap = cv2.VideoCapture(video_path)
        H = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        W = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        segments = []
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        results = model.predict(source=video_path, stream=True, save=False)
        from tqdm import tqdm
        for result in tqdm(results, desc="Extracting lane segments", total=total_frames):
            if result.masks is not None:
                ...
                for i in range(len(result.boxes)):
                    ...
                    # if not is_solid_lane(class_label): continue
    ...
    @classmethod
    def can_merge(cls, segA, segB, angle_thresh_deg=20, dist_thresh=40):
        a1, a2 = segA[0], segA[-1]
        b1, b2 = segB[0], segB[-1]
        pairs = [(a2, b1), (a2, b2), (a1, b1), (a1, b2)]
        for pA, pB in pairs:
            if np.linalg.norm(np.array(pA) - np.array(pB)) < dist_thresh:
                vecA = cls.direction_vector(a1, a2)
                vecB = cls.direction_vector(b1, b2)
    ...
    def run_eval(
        ...
        lane_model = YOLO(self.lane_model_path)
        segments, (H, W) = self.extract_lane_segments_all_frames(lane_model, video_path)
        lane_polygons = self.merge_segments(segments, self.angle_thresh_deg, self.dist_thresh)
        ...
        if save_overlay:
            ...
            if ret:
                ...
                overlay_img = self.draw_lanes_on_frame(first_frame, lane_polygons)
        ...
        lane_change_events = detector.process_video(
            video_path,
...
# laneval = LanePathEval()
# result = laneval.run_eval("input.mp4", output_path="output.mp4")
# print(result)
